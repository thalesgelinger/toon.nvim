                ...
                if type(v) == "table" then
                    if #v > 0 then
                        ...
                        if arr_str:match("^%[%d+%]: ") then
                            table.insert(obj_lines, key .. ": " .. arr_str:sub(arr_str:find(": ") + 2))
                        else
                        ...
                        if is_empty then
                            ...
                            for line in nested:gmatch("[^\n]+") do
                                table.insert(obj_lines, current_indent .. indent_str .. line)
                            end
                        end
                    end
                else
...
function M.json_to_toon(json_str)
    -- Parse JSON using vim.json.decode
...
-- Visual mode command implementation
function M.convert_selection()
    ...
    -- Join lines to form JSON string
    local json_str = table.concat(lines, "\n")

    -- Convert to Toon
    local success, result = pcall(M.json_to_toon, json_str)

    if not success then
        vim.notify("Toon conversion failed: " .. tostring(result), vim.log.levels.ERROR)
        return
    end
    ...
    for line in result:gmatch("([^\n]+)") do
        table.insert(toon_lines, line)
    end

    -- Replace selected lines with Toon format
    vim.api.nvim_buf_set_lines(0, start_line - 1, end_line, false, toon_lines)

    vim.notify("Converted to Toon format", vim.log.levels.INFO)
end

-- Setup function to create the command
function M.setup()
    vim.api.nvim_create_user_command('Toon', function()
        M.convert_selection()
    end, { range = true, desc = 'Convert selected JSON to Toon format' })
end
